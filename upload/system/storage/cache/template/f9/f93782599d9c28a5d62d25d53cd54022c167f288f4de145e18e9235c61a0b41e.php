...
class __TwigTemplate_81bcd3a706364183cfabe3fa46ab76fab673bd47564036f6f753fe99dd484c93 extends \Twig\Template
{
    private $source;
    private $macros = [];
...
function buildConditionHTML(condNode) {
    ...
    function renderValueInput(fieldMeta, currentVal) {
        ...
        echo "',
            ...
            var \$acInput = \$('<input type=\"text\" class=\"form-control condition-autocomplete\" style=\"display:inline-block; width:180px; margin-right:5px;\" placeholder=\"Search...\" />');
            var \$hiddenVal = \$('<input type=\"hidden\" class=\"condition-value\" />');
            \$hiddenVal.val(currentVal || '');
            
            if (currentVal) {
                var infoUrl = (fieldMeta.type === 'category') ? 'getCategoryInfo' : 'getProductInfo';
                \$.ajax({
                    url: 'index.php?route=extension/module/customer_segment/' + infoUrl + '&user_token=";
        // line 1021
        echo ($context["user_token"] ?? null);
        echo "&ids=' + currentVal,
                    dataType: 'json',
            ...
            \$acInput.autocomplete({
                'source': function(request, response) {
                    var route = (fieldMeta.type === 'category') ? 'catalog/category/autocomplete' : 'catalog/product/autocomplete';
                    \$.ajax({
                        url: 'index.php?route=' + route + '&user_token=";
        // line 1035
        echo ($context["user_token"] ?? null);
        echo "&filter_name=' + encodeURIComponent(request),
                        dataType: 'json',
                        success: function(json) {
                            response(\$.map(json, function(item) {
                                return {
                                    label: item['name'],
                                    value: (fieldMeta.type === 'category') ? item['category_id'] : item['product_id']
                                }
                            }));
                        }
                    });
                ...
                'select': function(item) {
                    \$acInput.val(item['label']);
                    \$hiddenVal.val(item['value']);
                }
            });

            \$valWrapper.append(\$acInput).append(\$hiddenVal);
        } else {
